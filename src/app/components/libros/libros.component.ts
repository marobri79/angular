...
export class LibrosComponent implements OnDestroy, OnInit {
  ...
  getUrlPhoto(libro: LibroDTO) {
    let urlPhoto = "";
    if (libro.identificadorImagen == null) {
      urlPhoto = `${environment.API_ENDPOINT}/imagenes/default.jpg`;
    }
    else {
      urlPhoto = `${environment.API_ENDPOINT}/imagenes/${libro.identificadorImagen}`;
    }
